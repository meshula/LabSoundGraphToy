...
#define RGBA_LE(col) (((col & 0xff000000) >> (3 * 8)) + ((col & 0x00ff0000) >> (1 * 8)) + ((col & 0x0000ff00) << (1 * 8)) + ((col & 0x000000ff) << (3 * 8)))
  ...
  static uint32_t colors[16] = { Colors::turqoise, Colors::greenSea, Colors::emerald, Colors::nephritis,
    Colors::peterRiver, Colors::belizeHole, Colors::amethyst, Colors::wisteria, Colors::sunFlower, Colors::orange,
    Colors::carrot, Colors::pumpkin, Colors::alizarin, Colors::pomegranate, Colors::clouds, Colors::silver
  };
  ...
  public:
    ...
    // load data for the current frame, at currFrameIndex.
    // timings are relative to frame start, not global
    void LoadFrameData(const legit::ProfilerTask *tasks, size_t count)
    {
      ...
      for (size_t taskIndex = 0; taskIndex < count; taskIndex++)
      {
        ...
        {
          // if the task under consideration has a different color or name add it
          ...
          {
            // otherwise extend the last existing task to include this one
    ...
    void RenderTimings(int graphWidth, int legendWidth, int height, float maxFrameTime, int frameIndexOffset)
    {
  ...
  private:
    ...
    void RenderGraph(ImDrawList *drawList, vec2 graphPos, vec2 graphSize, float maxFrameTime, size_t frameIndexOffset)
    {
      ...
      for (size_t frameNumber = 0; frameNumber < frames.size(); frameNumber++)
      {
        ...
        vec2 framePos = graphPos + vec2(graphSize.x - 1 - frameWidth - (frameWidth + frameSpacing) * frameNumber, graphSize.y - 1);
    ...
    void RenderLegend(ImDrawList *drawList, vec2 legendPos, vec2 legendSize, float maxFrameTime, size_t frameIndexOffset)
    {
      ...
      for (size_t taskIndex = 0; taskIndex < currFrame.tasks.size(); taskIndex++)
      {
        ...
        vec2 markerRightRectMin = legendPos + vec2(markerLeftRectMargin + markerLeftRectWidth + markerMidWidth, legendSize.y - markerRigthRectMargin - (markerRightRectHeight + markerRightRectSpacing) * stat.onScreenIndex);
    ...
    static void Rect(ImDrawList *drawList, vec2 minPoint, vec2 maxPoint, uint32_t col, bool filled = true)
    {
      if(filled)
    ...
    static void Text(ImDrawList *drawList, vec2 point, uint32_t col, const char *text)
    {
      drawList->AddText(ImVec2(point.x, point.y), col, text);
    }
    static void Triangle(ImDrawList *drawList, std::array<vec2, 3> points, uint32_t col, bool filled = true)
    {
      if (filled)
  ...
  public:
    ...
    void Render(float maxFrameTime)
    {
      fpsFramesCount++;
      ...
      gpuGraph.RenderTimings(graphWidth, legendWidth, graphHeight, maxFrameTime, frameOffset);
      cpuGraph.RenderTimings(graphWidth, legendWidth, graphHeight, maxFrameTime, frameOffset);
